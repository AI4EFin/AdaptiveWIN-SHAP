...
plt.plot(data_df.index, data_df.N, color='#3B75AF')
...
rolling = df["windows_mean"].rolling(window=10).mean()
plt.figure(figsize=(10,6))
plt.plot(data_df.index[50:], df['windows_mean'][50:], color='#3B75AF', label="Window Size")
plt.plot(data_df.index[50:], rolling[50:], color='red', label="Window Size Moving Average (10)")
