                ...
                for k in range(1, K + 1):
                    ...
                    try:
                        ...
                        for b in range(num_bootstrap):
                            ...
                            likelihood_i_b, _, _, _, _, _ = self.model.fit(X_all, y_star).diagnostics(X_all, y_star)
                            ...
                            for i_abs in J_abs:
                                ...
                                if mA < 20 or mB < 20:  # same min-seg rule
                                    continue
                                likelihood_a_b, _, _, _, _, _ = self.model.fit(X_all[Lmask], y_star[Lmask]).diagnostics(X_all[Lmask], y_star[Lmask])
                                likelihood_b_b, _, _, _, _, _ = self.model.fit(X_all[Rmask], y_star[Rmask]).diagnostics(X_all[Rmask], y_star[Rmask])
